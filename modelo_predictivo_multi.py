...
"""
================================================================================
Modelo SARIMAX Multi-Estación para PM2.5 en Bogotá
================================================================================

Proyecto Interdisciplinar - Universidade Nacional da Colômbia
Autores: Karen Daniela Marin Baez e Johann Camilo Rincon Real

Análisis de TODAS las estaciones de la RMCAB con visualización integrada.
...
# Umbral mínimo de datos para considerar una estación válida
...
def get_valid_stations(df, min_points=MIN_DATA_POINTS):
    """Obtiene lista de estaciones válidas (sin NowCast ni IBOCA)."""
    stations = []
    for col in df.columns:
        if '_NowCast' not in col and '_IBOCA' not in col:
...
# =============================================================================
# PIPELINE POR ESTACIÓN
# =============================================================================

def process_single_station(station, df, clima_dict, fire_dict, test_hours=168):
    """Procesa una sola estación."""
    try:
        print(f"\n{'='*60}")
        ...
        if len(serie) < 1000:
            print(f"  ⚠ Datos insuficientes: {len(serie)} obs")
            return None
        ...
        # Predicción hasta diciembre 2025
...
# =============================================================================
# VISUALIZACIÓN MULTI-ESTACIÓN
# =============================================================================

def create_multi_station_visualization(all_results, output_dir):
    """Crea HTML con resumen de todas las estaciones."""
    ...
    fig = make_subplots(
        rows=n_rows, 
        cols=2,
        subplot_titles=[r['station'].replace('___', ' - ').replace('_', ' ') for r in all_results],
    ...
    for idx, result in enumerate(all_results):
        ...
        hist = result['serie'][-720:]  # 30 días * 24 horas
        fig.add_trace(
            go.Scatter(
                x=hist.index, 
                y=hist.values,
                name='Observado',
                line=dict(color=color_hist, width=1),
                showlegend=(idx == 0),
                legendgroup='hist'
            ),
            row=row, col=col
        ...
        fig.add_trace(
            go.Scatter(
                x=result['future_dates'][:168],  # Próxima semana
                y=result['future_mean'].values[:168],
                name='Predicción',
                line=dict(color=color_pred, width=2),
                showlegend=(idx == 0),
                legendgroup='pred'
            ),
            row=row, col=col
        ...
        fig.add_trace(
            go.Scatter(
                x=list(result['future_dates'][:168]) + list(result['future_dates'][:168][::-1]),
                y=list(result['future_ci_upper'].values[:168]) + list(result['future_ci_lower'].values[:168][::-1]),
                fill='toself',
                fillcolor=color_ci,
        ...
        fig.update_yaxes(title_text="PM2.5 (µg/m³)", row=row, col=col)
    
    # Layout general
    fig.update_layout(
        title_text="Predicción PM2.5 - Todas las Estaciones RMCAB (2023-2025)<br><sub>Últimos 30 días observados + Próxima semana predicha</sub>",
        height=400 * n_rows,
        showlegend=True,
    ...
    fig.write_html(os.path.join(output_dir, 'resumen_todas_estaciones.html'))
    print(f"\n✓ Resumen guardado en {output_dir}/resumen_todas_estaciones.html")
    
    # Crear gráficos individuales detallados
    for result in all_results:
        create_individual_plot(result, output_dir)


def create_individual_plot(result, output_dir):
    """Crea gráfico individual para cada estación."""
    fig = go.Figure()
    ...
    # Predicción test
    fig.add_trace(go.Scatter(
        x=result['test_y'].index, 
        y=result['test_pred'].values,
    ...
    fig.add_trace(go.Scatter(
        ...
        y=list(result['future_ci_upper'].values) + list(result['future_ci_lower'].values[::-1]),
        fill='toself', 
    ...
    fig.add_hline(y=35, line_dash="dash", line_color="red", annotation_text="Límite OMS (35 µg/m³)")
    fig.add_hline(y=15, line_dash="dash", line_color="green", annotation_text="Meta OMS (15 µg/m³)")
    
    station_name = result['station'].replace('___', ' - ').replace('_', ' ')
    fig.update_layout(
        title=f'SARIMAX - {station_name} | MAE: {result["metrics"]["mae"]:.1f} µg/m³',
        xaxis_title='Fecha',
    ...
    filename = f"grafico_{result['station']}.html"
    fig.write_html(os.path.join(output_dir, filename))


def export_all_data(all_results, output_dir):
    """Exporta datos de todas las estaciones."""
    os.makedirs(output_dir, exist_ok=True)
    ...
    for result in all_results:
        station_data = {
            ...
            'alerts': {
                'critical': len([a for a in result['alerts'] if a['level'] == 'critical']),
                'moderate': len([a for a in result['alerts'] if a['level'] == 'moderate'])
            },
            'historical_mean': round(float(result['serie'].mean()), 1),
            'forecast_mean': round(float(result['future_mean'].mean()), 1)
        }
        summary['stations'].append(station_data)
    
    # Guardar resumen
    with open(os.path.join(output_dir, 'resumen_estaciones.json'), 'w') as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    
    # Guardar datos completos de cada estación
    for result in all_results:
        station_file = f"datos_{result['station']}.json"
        data = {
            ...
            'forecast': {
                'dates': [d.isoformat() for d in result['future_dates']],
                'values': [round(float(v), 1) for v in result['future_mean'].values],
                'ci_lower': [round(float(v), 1) for v in result['future_ci_lower'].values],
                'ci_upper': [round(float(v), 1) for v in result['future_ci_upper'].values]
            },
            'alerts': result['alerts']
        }
        
        with open(os.path.join(output_dir, station_file), 'w') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
    
    print(f"✓ Datos exportados para {len(all_results)} estaciones")
...
def main():
    print("\n" + "="*70)
    print("   MODELO SARIMAX MULTI-ESTACIÓN (2023-2025)")
    print("="*70)
    ...
    stations = get_valid_stations(df)
    print(f"\n✓ Estaciones válidas encontradas: {len(stations)}")
    ...
    for station in stations:
        result = process_single_station(station, df, clima_dict, fire_dict)
        if result is not None:
            all_results.append(result)
    
    if not all_results:
        print("\n⚠ No se pudo procesar ninguna estación")
        return
    
    print(f"\n✓ Procesadas exitosamente: {len(all_results)} estaciones")
    ...
    create_multi_station_visualization(all_results, OUTPUT_DIR)
    export_all_data(all_results, OUTPUT_DIR)
    ...
    print(f"\nEstaciones procesadas: {len(all_results)}")
    print(f"\nMAE promedio: {np.mean([r['metrics']['mae'] for r in all_results]):.2f} µg/m³")
    print(f"RMSE promedio: {np.mean([r['metrics']['rmse'] for r in all_results]):.2f} µg/m³")
    
    print(f"\nArchivos generados:")
    print(f"  - resumen_todas_estaciones.html (resumen visual)")
    print(f"  - resumen_estaciones.json (métricas)")
    print(f"  - grafico_[estacion].html ({len(all_results)} gráficos individuales)")
    print(f"  - datos_[estacion].json ({len(all_results)} archivos de datos)")
    
    # Top 5 estaciones por alertas críticas
    sorted_by_alerts = sorted(all_results, 
                             key=lambda x: len([a for a in x['alerts'] if a['level'] == 'critical']), 
                             reverse=True)
    
    print(f"\nTop 5 estaciones con más alertas críticas:")
    for i, r in enumerate(sorted_by_alerts[:5], 1):
        n_critical = len([a for a in r['alerts'] if a['level'] == 'critical'])
        print(f"  {i}. {r['station']}: {n_critical} alertas")
